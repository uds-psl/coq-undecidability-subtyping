...
Definition sum_map {A B C D} (f : A -> C) (g : B -> D) (p : sum A B) :
  sum C D.
Proof.
  destruct p. left. apply f,a. right. apply g,b.
Defined.

Definition sum_id {A B} {f : A -> A} {g : B -> B} :
  (forall x, f x = x) -> (forall x, g x = x) -> forall p, sum_map f g p = p.
Proof.
  intros. destruct p; cbn; apply f_equal; auto.
Defined.

Definition sum_ext {A B C D} {f f' : A -> C} {g g': B -> D} :
  (forall x, f x = f' x) -> (forall x, g x = g' x) -> forall p, sum_map f g p = sum_map f' g' p.
Proof.
  intros. destruct p; cbn; f_equal; auto.
Defined.

Definition sum_comp {A B C D E F} {f1 : A -> C} {g1 : C -> E} { h1} {f2: B -> D} {g2: D -> F} {h2}:
  (forall x, (funcomp  g1 f1) x = h1 x) -> (forall x, (funcomp g2 f2) x = h2 x) -> forall p, sum_map g1 g2 (sum_map f1 f2 p) = sum_map h1 h2 p.
Proof.
  intros. destruct p; cbn; f_equal; auto.
  now rewrite <- H. now rewrite <- H0.
Defined.
